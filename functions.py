...
from tkinter import *
def clear(): return os.system('cls')  # lambda function to clear prompt
...
def input_function():
    ...
    if global_variables.altitude_choice == 1:
        global_variables.altitude = user_input(
            global_variables.altitude, "altitude", "m")
    ...
    if global_variables.altitude_choice == 2:
        global_variables.density = user_input_0(
            global_variables.density, "Density", "kg/m^3")
        global_variables.dynamic_viscosity = user_input_0(global_variables.dynamic_viscosity, "Dynamic Viscosity",
                                                          "Pa*s")
        global_variables.pressure = user_input_0(
            global_variables.pressure, "Pressure", "kPa")
    ...
    if global_variables.thermal_choice == 1:
        ...
        global_variables.temperature = user_input(
            global_variables.temperature, "Temperature", "Celsius")
    ...
    # CALCULATIONS
    # KINEMATIC VISCOSITY
    global_variables.kinematic_viscosity = global_variables.dynamic_viscosity / \
        global_variables.density
    ...
    reynolds_downstream = global_variables.velocity * global_variables.density * global_variables.domain_x / \
        global_variables.dynamic_viscosity
    boundary_layer_thickness = 0.37 * \
        global_variables.domain_x / pow(reynolds_downstream, 0.2)
    ...
    global_variables.turbulence_intensity = 0.16 * \
        pow(global_variables.reynolds, -0.125) * 100
...
def permeability_calculator():
    ...
    while not global_variables.error:
        try:
            global_variables.input_permeability = float(
                input("Enter the open area in as a percentage [%]: "))
            global_variables.input_porous_thickness = float(
                input("Enter the modeled porous domain thickness [m]: "))
            ...
            if global_variables.input_permeability <= 0 or global_variables.input_porous_thickness <= 0:
                print(
                    "Permeability can't be lower than 0 or Porous thickness can't be 0 or lower")
            # Exit the loop if conditions are met
    ...
    print("Permeability resistance: ", '{:.2f}'.format(
        global_variables.permeability_resistance))
...
def print_function():
    ...
    if global_variables.altitude_choice == 1:
        print(f"Density [kg/m^3]: {global_variables.density:.3f}")
        print(
            f"Dynamic Viscosity [Pa*s]: {global_variables.dynamic_viscosity:.2e}")
        print(f"Pressure [kPa]: {global_variables.pressure:.3f}")
    print(
        f"Kinematic Viscosity [m^2/s]: {global_variables.kinematic_viscosity:.2e}")
    print(f"Reynolds Number [-]: {global_variables.reynolds:.2e}")
    print(
        f"Turbulence Intensity [%]: {global_variables.turbulence_intensity:.3f}")
    print(
        f"Turbulence Length Scale [m]: {global_variables.turbulence_length_scale:.5f}")
    print(
        f"Max Turbulence Length Scale [m]: {global_variables.max_turbulence_length_scale:.2f}")
    print(
        f"Turbulence Kinetic Energy (k) [m^2/s^2]: {global_variables.turbulence_kinetic_energy:.3f}")
    print(
        f"Turbulence Dissipation Rate (epsilon) [m^2/s^3]: {global_variables.turbulence_dissipation_rate:.5f}")
    print(
        f"Turbulence Specific Dissipation Rate (omega) [1/s]: {global_variables.turbulence_specific_dissipation_rate:.5f}")
    # Control if the temperature input is selected or not
    if global_variables.thermal_choice == 1:
        print(
            f"Thermal Conductivity [W/(m.K)]: {global_variables.thermal_conductivity:.5f}")
        print(f"Prandtl number [-]: {global_variables.prandtl:.3f}")
        print(
            f"Isobaric specific heat [kJ/(kg.K)]: {global_variables.isobaric_specific_heat:.3f}")
    ...
    while True:
        try:
            ...
            root.filename = filedialog.asksaveasfilename(
                initialdir=global_variables.current_directory, title="Save as", filetypes=(("txt files", "*.txt"), ("all files", "*.*")))
            root.destroy()
            ...
            f.write("Inputs:\n"
                    ...
                    "Domain size (X): "
                    + str(global_variables.domain_x) + " [m]\n"
                    "Domain_size (Y): "
                    + str(global_variables.domain_y) + " [m]\n"
                    "Domain_size (Z): " + str(global_variables.domain_z) + " [m]\n")
            if global_variables.altitude_choice == 1:
                a.write("Altitude: "
                        + str(global_variables.altitude) + " [m]\n")
            ...
            if global_variables.thermal_choice == 1:
                a.write("Temperature: "
                        + str(global_variables.temperature) + " [Celsius]\n")
            ...
            if global_variables.altitude_choice == 1 and global_variables.thermal_choice == 1:
                a.write("Density: " + str('{:.3f}'.format(global_variables.density)) + " [kg/m^3]\n"
                        "Dynamic Viscosity: "
                        + str('{:.7f}'.format(global_variables.dynamic_viscosity)) + " [Pa*s]\n"
                        "Kinematic Viscosity: "
                        + str('{:.7f}'.format(global_variables.kinematic_viscosity)) + " [m^2/s]\n"
                        "Pressure: "
                        + str('{:.3f}'.format(
                            global_variables.pressure)) + " [kPa]\n"
                        "Prandtl number: "
                        + str('{:.3f}'.format(global_variables.prandtl)
                              ) + " [-]\n"
                        "Thermal Conductivity: "
                        + str('{:.5f}'.format(global_variables.thermal_conductivity))
                        + " [W/(m.K)]\n"
                        "Isobaric specific heat: "
                        + str('{:.3f}'.format(global_variables.isobaric_specific_heat))
                        + " [kJ/(kg.K)]\n")
            ...
            if global_variables.altitude_choice == 2 and global_variables.thermal_choice == 1:
                a.write("Pressure: " + str('{:.3f}'.format(global_variables.pressure)) + " [kPa]\n"
                        "Thermal Conductivity: "
                        + str('{:.3f}'.format(global_variables.thermal_conductivity))
                        + " [W/(m*K)]\n"
                        "Specific heat constant (Cp): " + str(
                            '{:.3f}'.format(global_variables.isobaric_specific_heat))
            ...
            a.write("Reynolds Number: " + str('{:.0f}'.format(global_variables.reynolds)) + " [-]\n"
                    "Turbulence Intensity: "
                    + str('{:.3f}'.format(global_variables.turbulence_intensity)) + " [%]\n"
                    "Turbulence Length Scale: "
                    + str('{:.5f}'.format(global_variables.turbulence_length_scale)) + " [m]\n"
                    f"Max Turbulence Length Scale: {global_variables.max_turbulence_length_scale:.1f} [%]\n"
                    "Turbulence Kinetic Energy (k): " + str(
                         '{:.5f}'.format(global_variables.turbulence_kinetic_energy))
                    + " [m^2/s^2]\n"
            ...
            if global_variables.input_yplus != 0:
                a.write("\nWall Space Calculation\n"
                        "Input:\n"
                        "Desired y+ value: "
                        + str(global_variables.input_yplus) + " [-]\n"
                        "Output:\n"
                        "Required First Layer Thickness: "
                        + str('{:.3f}'.format(global_variables.first_layer_thickness))
                        + " [m]\n")
            # Check if user selected the function
            if global_variables.input_permeability != 0 and global_variables.input_porous_thickness != 0:
                a.write("\nPermeability Calculation\n"
                        ...
                        + str(global_variables.input_permeability) + " [%]\n"
                        "Geometry Thickness: "
                        + str(global_variables.input_porous_thickness)
                        + " [m]\n"
                        "Output:\n"
                        "Resistance coefficient: "
                        + str('{:.3f}'.format(global_variables.permeability_resistance))
                        + " [1/m]\n")
...
def wall_space_calculator():
    ...
    while not global_variables.error:
        try:
            global_variables.input_yplus = float(
                input("Enter your desired y+ value: "))
            # To check if the input value is not lower than 0
            if global_variables.input_yplus <= 0:
    ...
    wall_shear = global_variables.density * \
        friction_coefficient * pow(global_variables.velocity, 2) / 2
    ...
    print("First layer thickness[m]: ", '{:.5f}'.format(
        global_variables.first_layer_thickness))
        ...
        while True:
            try:
                print("Turbulence Model:\n"
                      ...
                      + "kOmegaSST \t(PRESS 2)")
                # Assign the input as int to the variable
                global_variables.incompressible_ras_turbulence_model = int(
                    input("Your choice: "))
                # Exit the loop if conditions are met
    ...
    while True:
        try:
            print("Time dependency:\n"
                  ...
                  # export wall conditions
                  mesh_export.bc_reader("wall", global_variables.wall_output)
                  # export inlet conditions
                  mesh_export.bc_reader(
                      "velocity-inlet", global_variables.inlet_output)
                  # export outlet conditions
                  mesh_export.bc_reader(
                      "pressure-outlet", global_variables.outlet_output)
                  # export symmetry conditions
                  mesh_export.bc_reader(
                      "symmetry", global_variables.symmetry_output)
                  # create the openfoam input directory
                  file_creator.create_working_directory(R"\openfoam_inputs")
                  file_creator.create_working_directory(
                      R"\openfoam_inputs\0")  # create the 0 directory
